    ...
    <section id="about" className="relative bg-white px-6 py-20 md:px-12 lg:px-20">
      <div className="mx-auto max-w-7xl">
        ...
        >
          ...
          {/* Right: References Button (desktop only) */}
          <button
            onClick={() => handleTileClick("references")}
            className="hidden md:flex items-center gap-2 px-6 py-2.5 bg-white/60 hover:bg-white/75 border border-white/40 hover:border-white/60 rounded-lg transition-colors font-semibold text-ink whitespace-nowrap h-fit"
          >
            References
            <svg
              className="h-4 w-4"
        ...
        </motion.div>          {/* Content area - shows grid or expanded view */}
          {!activeId ? (
            // Poster grid
            <motion.div
              animate={{
